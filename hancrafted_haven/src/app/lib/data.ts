...
export async function fetchProductData(
  ...
  try {
    ...
    const result =
      await sql`SELECT id, creator_id, name, description, price, category FROM products WHERE id = ${productId}`;
...
export async function fetchImagesData(productId: number): Promise<Image_[]> {
  try {
    // Fetching the product by id
    const result =
      await sql`SELECT id, product_id, image_url FROM product_images WHERE product_id = ${productId}`;
